...
int decode_outputs(std::vector<float> &grid_centers_w, std::vector<float> &grid_centers_h, 
    ...
    {   
        ...
        out_string <<  "\"info\":\"no face or more than 1 faces\"";
        out_string << "}";
        return 0;
    }
...
int main(int argc, char** argv) {
    ...
    error_log.open("/opt/project/log/peoplene_log.txt", std::ios::out);
    if(!error_log)
    {
    ...
    int img_w = img.cols;
    int img_h = img.rows;
    std::cout << input_file + result_name << std::endl;
    ...
    {   
        ...
        std::string http_string = out_string.str();
        http_string[0] = '[';
        std::cout <<"env: " << http_string  << " " << WARNING_OUT_HTTP2<< std::endl;
        char warning_http[512];
        sprintf(warning_http,WARNING_OUT_HTTP2,device_param.http_trans.c_str(),device_param.http_port);
    ...
    else
    {
        ...
        out_string <<  "\"info\":\"face registration successful\"";
        out_string << "}]";
        std::string http_string = out_string.str();
        std::cout <<"env: " << http_string << std::endl;
        char warning_http[512];
